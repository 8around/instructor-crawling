...
def get_html_tbody(page_number = 1):
    ...
    # 크롤링할 웹 페이지 URL
    url = "https://www.gwanak.go.kr/site/gwanak/ex/bbsNew/List.do?typeCode=1"
    ...
    try:
        ...
        if page_number != 1:
            for i in range(2, page_number + 1):
                ...
                link = driver.find_element(By.XPATH, f'//a[text()="{i}"]')
                link.click()
...
def extract_post_data(title, date, link):
    ...
    link = link[12:17]
    full_url = 'https://www.gwanak.go.kr/site/gwanak/ex/bbsNew/View.do?not_ancmt_mgt_no=' + link + '&typeCode=1'

    post = get_soup(full_url)
    content = post.find('div', class_ ='txt-area')
...
try:
    ...
    for tbody in tbody_list:
        titles = tbody.select('tbody > tr> td:nth-child(3) > a')
        dates = tbody.select('tbody > tr> td:nth-child(4)')
