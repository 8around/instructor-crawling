...
def get_html_tbody(page_number=1):
    ...
    try:
        for i in range(1, page_number + 1):
            url = f"https://ycsupport.or.kr/bbs/board.php?bo_table=recruitment&sfl=wr_subject&stx=%EA%B0%95%EC%82%AC&sop=and&page={i}"
            driver.get(url)
...
try:
    ...
    for tbody in tbody_list:
        titles = tbody.select('tbody > tr > td.td_subject > div > ul.bo_tit_ul2 > a')
        dates = tbody.select('tbody > tr > td.td_subject > div > ul.bo_tit_ul3 > li.date')
