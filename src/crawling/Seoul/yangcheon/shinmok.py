...
def get_html_tbody(page_number=1):
    ...
    try:
        for i in range(1, page_number + 1):
            url = f"http://www.shinmok.or.kr/news/notice.php?page={i}"
            driver.get(url)
...
def extract_post_data(title, date, link):
    ...
    link = link[-6:-2]
    full_url = 'http://www.shinmok.or.kr/news/notice_view.php?page=1&idx=' + link

    try:
        post = get_soup(full_url)
        content = post.find('td', class_='detail_part')
...
try:
    ...
    for tbody in tbody_list:
        titles = tbody.select('tbody > tr > td.txt_left > a')
